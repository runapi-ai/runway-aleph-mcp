<h1 align="center">Runway Aleph MCP Server</h1>

<p align="center">
  <strong>Create Runway Aleph AI tasks through RunAPI — 1 endpoint, 1 model — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
`@runapi.ai/runway-aleph-mcp` is a focused MCP server for the **Runway Aleph** model line on RunAPI.
It lets an MCP-compatible assistant create Runway Aleph tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `edit_video` | Yes | Create a Runway Aleph task (edit video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a Runway Aleph model and endpoint. |

---

## Models

Runway Aleph covers 1 model across 1 endpoint. Each tool accepts the models listed for it:
